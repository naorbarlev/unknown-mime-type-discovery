...
export {
	...
	redef enum Log::ID += { UnknownMimeTypeDiscovery::LOG };
...
event zeek_init()
	{
	Log::create_stream(UnknownMimeTypeDiscovery::LOG, [ $columns=Info,
...
event file_sniff(f: fa_file, meta: fa_metadata) &priority=-5
	...
	f$source != "SSL" && f$source != "DTLS" )
		{
		Log::write(UnknownMimeTypeDiscovery::LOG, Info($ts=network_time(), $fuid=f$id,
